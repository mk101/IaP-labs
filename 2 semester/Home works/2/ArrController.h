...
public:
	...
	ArrController(E* elems, D* degrees, uint len) {
		assert(len > 0);
		this->Length = len;
	...
	ArrController(const ArrController& ac) {
	...
	void Add(const E& elem, const D& deg) {
	...
	ArrController<E, D>& operator=(const ArrController& ac) {
		this->Length = ac.Length;
	...
	friend std::ostream& operator<<(std::ostream& out, const ArrController<E, D>& ac) {
	...
	friend std::istream& operator>>(std::istream& in, ArrController<E, D>& ac) {
		uint len = 0;
		in >> len;
